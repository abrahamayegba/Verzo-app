...
const SendPurchase: React.FC<SendPurchaseProps> = ({
  ...
  const [sendPurchaseMutation, { loading }] = useSendPurchaseMutation();
  ...
  const handleSendPurchaseClick = async () => {
    try {
      await sendPurchaseMutation({
        variables: { purchaseId: purchaseId, copy: copy },
      });
      onClose();
      setCopy(false);
      showSuccessToast();
              ...
              <Dialog.Panel className="relative transform gap-y-3 overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow transition-all sm:w-full sm:max-w-[450px] sm:p-7">
                <div className=" flex flex-col gap-y-3">
                  ...
                  <p className=" text-primary-greytext">
                    Send this purchase to {merchantName} at {merchantEmail}
                  </p>
                  ...
                  <div className=" flex justify-between mt-4">
                    ...
                    <button
                      type="button"
                      disabled={loading}
                      onClick={handleSendPurchaseClick}
