...
import {
  ...
  useCreateMerchantWithCsvMutation,
...
const UploadMerchantCSV: React.FC<UploadCSVProps> = ({ open, onClose }) => {
  const { toast } = useToast();
  const storedBusinessId = JSON.parse(
    localStorage.getItem("businessId") || "[]"
  );
  const businessId = storedBusinessId[0] || "";
  const [createMerchantWithCsvMutation, { data, loading, error }] =
    useCreateMerchantWithCsvMutation();
  ...
  const showSuccessToast = () => {
    toast({
      title: "Imported!",
      description: "Your merchants have been successfully imported",
      duration: 3500,
    });
  };
  ...
  const handleImportConfirm = async () => {
    try {
      const { data } = await createMerchantWithCsvMutation({
        variables: {
      ...
      if (data?.createMerchantWithCsv === true) {
        onClose();
            ...
            >
              <Dialog.Panel className="relative transform gap-y-3 overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow transition-all sm:w-full sm:max-w-[450px] sm:p-9">
                <div className=" flex flex-col gap-y-3">
                  ...
                  <p className=" text-lg text-[#121212]">Upload Merchants</p>
                  <p className=" text-primary-greytext">
                    Upload a CSV file to help us import your merchants.
...
export default UploadMerchantCSV;
