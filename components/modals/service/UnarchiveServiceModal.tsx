...
interface UnarchiveServiceProps {
  open: boolean;
  openModal: () => void;
  onClose: () => void;
  serviceId: string;
...
const UnarchiveService: React.FC<UnarchiveServiceProps> = ({
  ...
  const handleUnarchiveServiceClick = async () => {
    try {
      await unarchiveServiceByBusinessMutation({
            ...
            >
              <Dialog.Panel className="relative transform gap-y-3 overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow transition-all sm:w-full sm:max-w-[450px] sm:p-9">
                <div className=" flex flex-col gap-y-3">
                  ...
                  <p className=" text-lg text-[#121212]">Unarchive service</p>
                  <p className=" text-primary-greytext">
                    Are you sure you want to unarchive this service? It will be
                    visible
                  </p>
                  <div className=" flex justify-between mt-6">
                    ...
                    <button
                      type="button"
                      onClick={handleUnarchiveServiceClick}
