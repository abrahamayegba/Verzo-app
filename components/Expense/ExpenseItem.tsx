...
import CreateCustomCOASheet from "../sheets/expense/CreateCustomCOASheet";
...
const ExpenseItem: React.FC<ExpenseInputProps> = ({
  ...
  const [openCustomCOA, setOpenCustomCOA] = useState(false);
  const handleCloseCustomCOA = () => {
    setOpenCustomCOA(false);
  };
  ...
  return (
    <>
        ...
        <div className="flex flex-col w-1/8 gap-y-2">
          <div className="flex gap-x-3">
            <input
              ...
              onChange={(e) =>
                onExpenseChange(index, "quantity", e.target.value)
              }
            />
          </div>
        </div>
            ...
            >
              ...
              <SelectContent className=" bg-white w-[200px] max-h-[300px] overflow-y-scroll">
                <SelectGroup>
                  ...
                  <button
                    type="button"
                    onClick={() => setOpenCustomCOA(true)}
                    className=" pl-8 flex items-start py-2 text-[15px] hover:bg-gray-100 cursor-pointer w-full"
                  >
                    Create custom COA
                  </button>
                </SelectGroup>
      ...
      <CreateCustomCOASheet
        open={openCustomCOA}
        onClose={handleCloseCustomCOA}
      />
    </>
  );
};
