...
import { useUserSignUpAfterInviteMutation } from "@/src/generated/graphql";
...
const UserSignup = () => {
  ...
  const inviteIdParams = useSearchParams();
  const inviteId = inviteIdParams.get("inviteId")!;
  ...
  const [userSignUpAfterInviteMutation, { loading }] =
    useUserSignUpAfterInviteMutation();
  ...
  const SignUpHandler = async (form: FormData) => {
    try {
      const response = await userSignUpAfterInviteMutation({
        variables: {
          inviteId: inviteId,
          ...form,
        },
      });
      saveToken(response.data?.userSignUpAfterInvite.access_token!);
      router.push(`/auth/verifyemail?hasBusiness=true`);
    } catch (error: any) {
      console.error(error);
